...
(defun as-exp-database (x f)
  (let* ((table *exp-database*)
         (ixu (f-to-i x))
         (a 0)
         (b (1- (first (array-dimensions table))))
         (m (floor (+ a b) 2)))
    (loop while (<= a b)
          do (let* ((cmf (aref table m))
                    (cmu (f-to-i cmf)))
               (cond ((< cmu ixu)
                      (setf a (1+ m)))
                     ((= cmu ixu)
                      (let* ((s20 #x57f5fe2e5bde4075)
                             (s21 #x3c1f16b8ed)
                             (s 333811522313371)
                             (jff f)
                             (jfu (f-to-i jff))
                             (dru (logior (ash (ash s (- m)) 63)
                                          #x3c90000000000000))
                             (m>>5 (ash m (- 5)))
                             (s2[m>>5] (if (zerop m>>5) s20 s21))
                             (m<<1 (ash m 1))
                             (m<<1&63 (logand m<<1 63))
                             (tt (logand (ash s2[m>>5] m<<1&63) 3)))
                        (loop for k from -1 to 1
                              for ru = (+ jfu k)
                              when (= (logand ru 3) tt)
                                do (return-from as-exp-database
                                     (+ (i-to-f ru) (i-to-f dru))))
                        (loop-finish)))
                     (t
                      (setf b (1- m))))
               (setq m (ash (+ a b) -1))))
