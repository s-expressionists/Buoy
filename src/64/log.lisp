...
;;;
;;; The above comment is a bit umclear.  What it means is that the
;;; original value of x has been modified so that 1 <= x < 2 by
;;; dividing it with 2^e.  If the original argument is a normal float,
;;; then this is easy.  But if it is a subnormal float, things have
;;; been adjusted so that the mantissa is normalized and the exponent
;;; has been decreased accordingly.  That's how 1 <= x < 2 is
;;; possible.
;;;
...
;;; In the core-match library, this application has a different
;;; interpretation of the value of a custom float 64 structure.  In
;;; other applications, the most significant bit in the HIGH slot
;;; means 1/2, whereas in this applications, it means 1.  But if we
;;; use a different interpretation, we would have to redefine lots of
;;; operators, so instead we are going to adapt the code to use the
;;; same interpretation as other applications do,

;;; I created this table by inferring a formula from the entries in
;;; the analogous table in log/dint.h.  But since I am using the
;;; interpretation of custom floats that other applications use, the
;;; entries in this table are going to be twice the entries in the
...
#+(or)(defparameter *inverse-table-2*
  (make-array
   240
   :initial-contents
   (cons (make-custom-float-64 :high #x8000000000000000 :low 0 :exponent 2 :sign 0)
