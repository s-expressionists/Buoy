...
;;; |x|<0.25.
;;;
;;; For other arguments the identity
;;; sinh(x)=(exp(|x|)-exp(-|x|))/2*copysign(1,x) is used.  Of course,
;;; it is not computed by first computing the two exponents and then
;;; dividing by 2, because computing the exponent might overflow, even
;;; though the final result does not.  So the two terms are computed
;;; so that each one is already divided by 2.
;;;
;;; For |x|<5 both exponents are calculated with slightly higher
;;; precision than double. For 5<|x|<36.736801 the exp(-|x|) is small
;;; and is calculated with double precision but exp(|x|) is calculated
;;; with higher than double precision. For 36.736801<|x|<710.47586
            ...
            (let ((rh 0d0)
              ...
              (if (> aix #x4014000000000000) ; |x| > 5
                       ...
                       (multiple-value-setq (th tl)
                         (sinh-as-exp-accurate ax 1t th tl))
                       (multiple-value-setq (th tl)
                         (fast-two-sum th tl))
                       (setf th (* th (copy-sign 1d0 x)))
                       (setf tl (* tl (copy-sign 1d0 x)))
                       (let* ((uhu (f-to-i th))
                              (ulu (f-to-i tl))
                 ...
                 (progn
                   ...
                   (let* ((q0l (aref t0 j0 0))
                     ...
                     (multiple-value-setq (qh ql)
                       (sinh-as-exp-accurate (- ax) (- t) qh ql))
                     (setf rh (- th qh))
              ...
              (let* ((q0h (aref t0 j0 1))
                ...
                (let* ((pm (evaluate-sinh-ch-polynomial mx dx2))
                  ...
                  (let* ((magic #.(parse-c-literal "0x1.c0ap-62"))
                    ...
                    (multiple-value-setq (th tl)
                      (sinh-sinh-as-exp-accurate ax tt th tl))
                    (multiple-value-setq (qh ql)
    ...
    (multiple-value-bind (1t il)
      ...
      (multiple-value-bind (je j0 j1)
        ...
        (let* (;; The exponents are computed minus 1, which is the
               ...
               (qh (* qh smf))
               (ql (* ql smf))
               (pm (evaluate-sinh-ch-polynomial mx dx2))
               (fph th)
               (fpl (+ tl (* th pp)))
               (fmh qh)
               (fml (+ ql (* qh pm)))
